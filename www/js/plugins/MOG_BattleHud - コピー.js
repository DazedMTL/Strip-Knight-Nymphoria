...
Game_Action.prototype.apply = function (target) {
	...
	if (target.isActor()) {
		if (oldhp > target.hp) {
			//		 	target._bhud_face_data = [60,0,0,60];
			target._bhud_face_data[0] = 60;
			target._bhud_face_data[1] = 0;
			target._bhud_face_data[3] = 60;
		}
...
Sprite_Actor.prototype.setupDamagePopup = function () {
	if (!$gameSystem.isSideView() && this._sprite_face) {
		if (this._battler.isDamagePopupRequested()) {
			...
			sprite.y = this.y + this.damageOffsetY() + 50;
...
Battle_Hud.prototype.refresh_face = function () {
	...
	var cw = this._face.bitmap.width / 3;
	var ch = this._face.bitmap.height / 2;
	var w = this._face_data[5] % 3;
	var h = parseInt(this._face_data[5] / 3);
	this._face.setFrame(cw * w, h * ch, cw, ch);
};
...
Battle_Hud.prototype.update_face_animation = function () {
	if (this._battler._bhud_face_data[3] > 0) {
		...
		if (this._battler._bhud_face_data[3] === 0) {
			if (this._battler.isDead()) { this._battler._bhud_face_data[2] = 5 }
			else {
				if (this._battler.hp <= 30 * this._battler.mhp / 100) { this._battler._bhud_face_data[2] = 2 }
				else if (this._battler.hp <= 60 * this._battler.mhp / 100) { this._battler._bhud_face_data[2] = 1 }
				else { this._battler._bhud_face_data[2] = 0 };
				if ($gameVariables.value(2) >= 100) this._battler._bhud_face_data[2] += 3;
			};
		};
	};
};
